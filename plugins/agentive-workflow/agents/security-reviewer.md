...
**📖 Complete Guide**: the `agentive-workflow:code-review-evaluator` skill
...
2. **Verify CI**: Use `/agentive-workflow:check-ci` slash command or run `./scripts/core/verify-ci.sh <branch>`
...
Security context is NOT baked into this distributed agent body. Read it
at session start from the repo you are reviewing (`CLAUDE.md`, the task
spec, `.kit/context/`), and establish before reviewing:
...
A reviewer without this context over-reports on threats the deployment
does not face and under-reports on the ones it does — if the repo does
not state it, ask the user before the review.
