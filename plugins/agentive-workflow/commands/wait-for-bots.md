...
> 🧭 `/agentive-workflow:wait-for-bots` — polls until BugBot and CodeRabbit have both
...
- **If exit 0**: Both bots have reviewed HEAD. Report the final status and
  suggest `/agentive-workflow:triage-threads` if unresolved threads exist.
- **If exit 1**: Timeout reached. Report which bot(s) are still STALE/MISSING
  and suggest manual investigation with `/agentive-workflow:check-bots`.
