...
> 🧭 `/agentive-workflow:check-spec` — traces every task requirement to code and tests
...
**When**: After tests pass + self-review, BEFORE `/agentive-workflow:commit-push-pr`.
...
- **PASS** — every requirement traced to code AND a test -> proceed to
  `/agentive-workflow:commit-push-pr`
