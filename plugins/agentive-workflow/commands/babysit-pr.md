...
> 🧭 `/agentive-workflow:babysit-pr` — monitors a PR through bot-review cycles: wait,
...
- **Exit 1**: Timeout. Report which bot is still missing/stale. **Stop babysitting** —
  ask the user to investigate manually with `/agentive-workflow:check-bots`.
...
If clean: suggest `/agentive-workflow:preflight` to run the full completion checklist before human review.
