...
# /agentive-workflow:wrap-up — Finalize Session
...
the remainder of `/agentive-workflow:wrap-up` regardless of what `## Target Repository` says
...
## Step 2: Run /agentive-workflow:retro

Invoke the `/agentive-workflow:retro` skill to capture session learnings. This saves the retro to `.kit/context/retros/<TASK-ID>-retro.md`.

The `/agentive-workflow:retro` command has its own cross-repo detection — it will automatically use the target repo for git/gh operations if configured.

If `/agentive-workflow:retro` fails (e.g., no PR found), note the failure but continue to
...
Every line is a claim — verify before printing it. If Step 2's `/agentive-workflow:retro`
failed, replace the retro line with the failure, e.g.:

```text
Retro: NOT WRITTEN — /agentive-workflow:retro failed (<one-line reason>)
```
