...
> 🧭 `/agentive-workflow:commit-push-pr` — stages your changes, commits, pushes, and
...
Parse the `GATE:` lines and present the PASS/FAIL table (same format as `/agentive-workflow:preflight`).
...
1. Wait for bots: `/agentive-workflow:check-bots` (CodeRabbit ~1-2 min, BugBot ~4-6 min)
2. Triage bot findings: `/agentive-workflow:triage-threads`
3. Run code-review evaluator (skill: code-review-evaluator)
4. Create review starter (skill: review-handoff)
5. Re-run preflight: `/agentive-workflow:preflight --pr PR_NUMBER --task TASK_ID`
