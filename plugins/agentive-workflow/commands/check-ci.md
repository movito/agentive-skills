...
> 🧭 `/agentive-workflow:check-ci` — verifies GitHub Actions status for a branch and
...
- Branch auto-detection reads from the target-repo working tree, so
  `/agentive-workflow:check-ci` without a branch argument finds the feature branch in
  `../target-repo`.
...
```text
/agentive-workflow:check-ci [branch-name] [--repo owner/name]
```
