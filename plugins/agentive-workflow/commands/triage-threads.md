...
> 🧭 `/agentive-workflow:triage-threads` — fetches all review threads on the PR,
...
After push: Run `/agentive-workflow:check-bots` to wait for re-scan, then re-run `/agentive-workflow:triage-threads` if new findings appear.
